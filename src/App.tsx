...
// ── Navigation types ──────────────────────────────────────────────────────────
type MainTab = 'warehouse-ops' | 'adaptive-twin' | 'autonomous-dock' | 'network-twin';
type SubView = 'overview' | 'simulation' | 'analytics' | 'ai';
...
const MAIN_TABS: { id: MainTab; label: string; Icon: typeof Activity; color: string }[] = [
  ...
  { id: 'network-twin',     label: 'Network Twin',     Icon: Network,   color: '#16a34a' },
];
...
// ── Top Navigation Bar ────────────────────────────────────────────────────────
function TopNav({
  mainTab, setMainTab, subView, setSubView,
}: {
  ...
  return (
    <header style={{ background: CARD, borderBottom: `1px solid ${BORDER}`, flexShrink: 0, zIndex: 10 }}>
      {/* Primary bar — logo + 4 main tabs */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', height: 52, borderBottom: `1px solid ${BORDER}` }}>
        {/* Logo */}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {isPlaying && (
            <span style={{ fontSize: 10, fontWeight: 700, background: '#f0fdf4', color: '#15803d', border: '1px solid #86efac', borderRadius: 99, padding: '3px 9px', display: 'flex', alignItems: 'center', gap: 4 }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#16a34a', display: 'inline-block' }} />
              Simulation Live
            </span>
          )}
          <span style={{ fontSize: 11, color: T3 }}>Jun 18, 2026</span>
        </div>
      </div>

      {/* Secondary bar — sub-tabs (only for warehouse-ops) */}
      {mainTab === 'warehouse-ops' && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 1, padding: '0 24px', height: 40 }}>
...
function ComingSoon({ tab }: { tab: MainTab }) {
  ...
  const descriptions: Record<MainTab, { tagline: string; bullets: string[] }> = {
    ...
    'network-twin': {
      tagline: 'Multi-site supply chain visibility — model ripple effects across the entire distribution network.',
      bullets: [
        'Cross-DC inventory positioning and transfer recommendations',
        'End-to-end lead time simulation from supplier to store shelf',
        'Risk scenario modeling: strikes, weather, port delays',
        'Network-wide cost optimisation across transport lanes',
      ],
    },
  };
...
export default function App() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', width: '100vw', overflow: 'hidden', background: PAGE }}>
      <TopNav mainTab={mainTab} setMainTab={setMainTab} subView={subView} setSubView={setSubView} />

      {mainTab === 'warehouse-ops' ? (
        <>
          {subView === 'overview'   && <Overview    setSubView={setSubView} />}
          {subView === 'simulation' && <SimulationView setSubView={setSubView} />}
          {subView === 'analytics'  && <AnalyticsView />}
          {subView === 'ai'         && <AIView setSubView={setSubView} />}
        </>
      ) : (
        <ComingSoon tab={mainTab} />
      )}
    </div>
  );
}
